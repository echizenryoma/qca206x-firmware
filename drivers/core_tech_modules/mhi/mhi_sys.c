...
void mhi_deinit_debugfs(struct mhi_device_ctxt *mhi_dev_ctxt)
{
	if (mhi_dev_ctxt->chan_info) {
		kfree(mhi_dev_ctxt->chan_info);
		mhi_dev_ctxt->chan_info = NULL;
	}

	if (mhi_dev_ctxt->child) {
		debugfs_remove_recursive(mhi_dev_ctxt->child);
		mhi_dev_ctxt->child = NULL;
	}
}
