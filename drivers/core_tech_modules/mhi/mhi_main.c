		   ...
		   struct mhi_chan_info *chan_info)
{
	...
	for (i = 0; i < sizeof(mhi_chan)/sizeof(*chan_info);
	     i++) {
		if (mhi_chan[i].chan_nr == chan) {
			memcpy(chan_info, mhi_chan+i, sizeof(*chan_info));
			return 0;
		}
	}
...
void mhi_deregister_device(struct mhi_device *mhi_device)
{
	...
	if (!mhi_ctxt) {
		pr_err("mhi_device is not registered\n");
		return;
	}


	mhi_deinit_debugfs(mhi_ctxt);
	mhi_ctxt_exit(mhi_ctxt);
