...
#ifdef CONFIG_NAPIER_X86
/* hard-coded from msm8996-ipcrouter.dtsi */
static int ipc_router_hsic_xprt_update_config(
	struct msm_ipc_router_hsic_xprt_config *hsic_xprt_config)
{
	int link_id;
	int version;
	char remote_ss[XPRT_NAME_LEN];

	strlcpy(hsic_xprt_config->ch_name, "ipc_bridge", XPRT_NAME_LEN);

	strlcpy(remote_ss, "external-modem", XPRT_NAME_LEN);
 ...
 */
static int msm_ipc_router_hsic_xprt_probe(void)
{
	int ret;
	struct msm_ipc_router_hsic_xprt_config hsic_xprt_config;
	...
	mutex_unlock(&hsic_remote_xprt_list_lock_lha1);
	ret = ipc_router_hsic_xprt_update_config(&hsic_xprt_config);
	if (ret) {
	...
	ret = msm_ipc_router_hsic_config_init(&hsic_xprt_config);
	if (ret) {
...
#ifdef CONFIG_NAPIER_X86
	rc = msm_ipc_router_hsic_xprt_probe();
#else
