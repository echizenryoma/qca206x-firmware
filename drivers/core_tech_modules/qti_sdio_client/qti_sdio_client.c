							...
							unsigned int bytes)
{
	...
	if (qsb->mode) {
		...
		if (ret == 1) {
			pr_info("operating in async mode now\n");
			goto out;
		}
		if (ret) {
			to_console = 1;
...
static int qti_client_probe(struct sdio_al_client_handle *client_handle)
{
	...
	if (client_handle->id == QCN_SDIO_CLI_ID_QMI) {
		...
		if (!ipc_pdev) {
			...
			pr_err("platform_device_alloc done for ipc_bridge_sdio\n");
		...
		if (ret) {
			...
			pr_err("platform_device_add done for ipc_bridge_sdio\n");
	...
	if (client_handle->id == QCN_SDIO_CLI_ID_DIAG) {
		...
		if (IS_ERR(diag_pdev)) {
			...
			pr_err("platform_device_register_data done for diag_bridge_sdio\n");
...
#ifdef CONFIG_NAPIER_X86
static int register_client(int id, int mode)
{
	...
	qsbdev[id]->id = id;
	if (id == 3) {
		qsbdev[id]->ch_name = "SDIO_AL_QMI_CH0";
		client_data->name = "SDIO_AL_CLIENT_QMI";
	}
	else if (id == 4) {
		qsbdev[id]->ch_name = "SDIO_AL_DIAG_CH0";
		client_data->name = "SDIO_AL_CLIENT_DIAG";
	}
	...
	pr_err("register sdio al client id:%d qti_bridge\n", client_data->id);
...
#else
	qsbdev[id]->ch_name = "SDIO_AL_TTY_CH0";
#endif
...
#else
	client_data->name = "SDIO_AL_CLIENT_TTY";
#endif
